...
public class Exercise_14_15_Stop_Sign extends Application {
	@Override // Override the start method in the Application class
	public void start(Stage primaryStage) {
		// Create a stack pane
		StackPane stackPane = new StackPane(); // multiple layers

		// Create a polygon and set its properties
		Polygon polygon = new Polygon();
		stackPane.getChildren().add(polygon);
		polygon.setFill(Color.RED);
		polygon.setRotate(22.5); // rotate the polygon 
		ObservableList<Double> list = polygon.getPoints();

		final double WIDTH = 200, HEIGHT = 200; // scene width and height 
		double centerX = WIDTH / 2, centerY = HEIGHT / 2; 
		double radius = Math.min(WIDTH, HEIGHT) * 0.4;

		// Add points to polygon list
		for (int i = 0; i < 8; i++) {
		 	list.add(centerX + radius * Math.cos(2 * i * Math.PI / 8)); // eight sides
